...
public class PagingUtil {
	...
	private int startSeq;            // 현재 블럭의 처음 글번호
	   ...
	   private StringBuffer reviewHtml;    // 페이징 관련 HTML
	   ...
	   public PagingUtil(String url, int currentPage, int totRecord, int blockCount, int blockPage) {
	      ...
	      if (currentPage > blockPage) {
	         pagingHtml.append("<b style='cursor:pointer' onclick='reviewPaging("  + (startBlock - 1) + ")'>");
	      ...
	      for (int i = startBlock; i <= endBlock; i++) {
	         ...
	         if (i == currentPage) {
	            pagingHtml.append(" <b id = 'currentPage' ><font color='red'>");
	            pagingHtml.append(i);
	            pagingHtml.append("</font></b>");
	         } else {
	            pagingHtml.append(" <b style='cursor:pointer' onclick='reviewPaging(");
	            pagingHtml.append(i);
	            pagingHtml.append(")'>");
	            pagingHtml.append(i);
	            pagingHtml.append("</b>");
	      ...
	      if (maxPage - startBlock >= blockPage) {
	         pagingHtml.append("<b style='cursor:pointer' onclick='reviewPaging("   + (endBlock + 1) + ")'>");
